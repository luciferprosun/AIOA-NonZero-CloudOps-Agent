"""Packaged fixed-process implementation of ``render_start_contract_v1``."""
...
_EGRESS_AUDIT_ENV: Final = "AIOA_W5_HERO_EGRESS_AUDIT_PATH"


class WorkspaceHeroRenderStartProfile:
    """Run exactly one AIOA-owned child with no caller-controlled process fields."""

    def run(self) -> TrustedRenderStartProfileResult:
        with tempfile.TemporaryDirectory(prefix="aioa-w5-hero-profile-") as directory:
            root = Path(directory)
            root.chmod(0o700)
            token_path = root / "operator.token"
            if not self._missing_token_fails_closed(root / "missing-token-probe"):
                raise TrustedRenderStartProfileFailure("MISSING_TOKEN_PROBE_FAILED")

            port = self._free_loopback_port()
            synthetic_value = "w5-synthetic-bootstrap-" + ("x" * 48)
            environment = self._fixed_environment(root, token_path, port)
            environment["AIOA_OPERATOR_TOKEN"] = synthetic_value
            ...
            if not (
                ...
                and token_path.read_text(encoding="utf-8") == f"{synthetic_value}\n"
            ):
            ...
            with (
                ...
                try:
                    ...
                    bootstrap_absent, child_argv_exact = self._inspect_child(process)
                except TimeoutError:
                    ...
                    if len(stdout) > _LOG_LIMIT_BYTES or len(stderr) > _LOG_LIMIT_BYTES:
                        raise TrustedRenderStartProfileFailure("RUNTIME_PROBE_OUTPUT_LIMIT")
                    if synthetic_value.encode() in stdout + stderr:
    ...
    def _missing_token_fails_closed(token_path: Path) -> bool:
        try:
            WorkspaceHeroRenderStartProfile._bootstrap(
    ...
    def _bootstrap(environment: dict[str, str]) -> dict[str, str]:
        ...
        bootstrap_value = child.pop("AIOA_OPERATOR_TOKEN", "")
    ...
    def _fixed_environment(root: Path, token_path: Path, port: int) -> dict[str, str]:
        ...
        probe_site = Path(__file__).resolve().with_name("_hero_probe_site")
        return {
            ...
            "PYTHONNOUSERSITE": "1",
            "PYTHONPATH": os.pathsep.join((str(probe_site), str(package_root))),
        }
    ...
    def _wait_for_runtime(
        ...
        while time.monotonic() < deadline:
            ...
            try:
                health = WorkspaceHeroRenderStartProfile._request_json(port, "/health")
                ready = WorkspaceHeroRenderStartProfile._request_json(port, "/ready")
    ...
    def _request_json(port: int, path: str) -> dict[str, object]:
        with urllib.request.urlopen(
            ...
            payload = response.read(_LOG_LIMIT_BYTES + 1)
            if len(payload) > _LOG_LIMIT_BYTES:
                raise ValueError("trusted runtime endpoint payload exceeds fixed limit")
    ...
    @staticmethod
    def _bounded_log(handle: object) -> bytes:
        handle.flush()  # type: ignore[attr-defined]
        handle.seek(0)  # type: ignore[attr-defined]
        return handle.read(_LOG_LIMIT_BYTES + 1)  # type: ignore[attr-defined,no-any-return]
