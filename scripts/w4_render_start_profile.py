"""Fixed repository-owned ``render_start_contract_v1`` runtime probe."""
...
from aioa_cloudops_agent.workspace.render_verification import (
...
class RenderStartContractV1Profile:
            ...
            with (
                ...
                try:
                    ...
                    if len(stdout) > _LOG_LIMIT_BYTES or len(stderr) > _LOG_LIMIT_BYTES:
                        raise TrustedRenderStartProfileFailure(
    ...
    def _fixed_environment(root: Path, token_path: Path, port: int) -> dict[str, str]:
        ...
        probe_site = Path(__file__).resolve().with_name("w4_probe_site")
        return {
